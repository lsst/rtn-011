\section{Rubin Construction Commissioning and Early System Optimization}
\label{sec:commissioning}

Rubin Construction  Commissioning was completed on \cstrcompdate, and the Observatory is now in the Rubin Operations Early  Optimization Phase.
Figure~\ref{fig:commissioning-es-schedule} presents the detailed schedule of completed commissioning and ongoing Early Science activities relative to the start of Operations.
\begin{figure}[htb]
\centering
\includegraphics[width=1.0\linewidth]{rubin_early_science_schematic}
\caption{Schematic showing the completed commissioning and ongoing Early Operations activities relative to the start of Operations, \opsstartdate.}
\label{fig:commissioning-es-schedule}
\vspace{0.1cm}
\end{figure}

As of \currentdate, Data Preview 2 (DP2) has been split into Early Data Preview 2 (EDP2), which will release the coadded images and all catalogs,
and Data Preview (DP2), which adds the visit-level data products, completing the full DP2 release.
This is discussed in detail in \S~\ref{ssec:dp2}.

\subsection{Summary of Commissioning Activities}
\label{ssec:commissioning-schedule}

The completion of Rubin Construction was marked by a series of key milestones, which are included here for completeness.
...
The Rubin on-sky commissioning campaign using  LSSTComCam began on \startsvcoomcam and ended on \finsvcomcam, lasting a total of 7 weeks, and included observations to support both engineering and science pipelines commissioning.
...
LSSTCam was installed on the Simonyi Survey Telescope on \cameraontma.
Following initial engineering and integration work and the achievement of the LSSTCam First Photon milestone, on-sky commissioning campaign using the LSSTCam began on \startonskylsstcam and ran through \finonskylsstcam.

The acquisition of science images\footnote{As opposed to images taken for engineering or Active Optics System (AOS) testing.} with LSSTCam began on \startsciencelsstcam.
Early observations consisted of  observations to support the Rubin First Look milestone followed by small-field survey visits dithered over small areas around a central boresight.
The small-field survey visits were typically acquired in sequences of at least 10 visits per bandpass, cycling through the available filters, in a manner similar to the small-field survey science visits acquired during LSSTComCam commissioning and  included in DP1.

The Science Validation (SV) surveys began on \startsv, acquiring visits in a manner consistent with the planned LSST operations cadence.
...
They covered a more limited sky area to achieve greater depth within the SV time frame, resulting in a temporal sampling distribution that differs from the LSST baseline.
...
The SV wide-area survey was conducted over a footprint that contracted progressively over the commissioning period, necessitated by limited observing time rather than reflecting the original survey design.
The survey began with an initial area of approximately 3,000~deg$^{2}$ (the ``3K'' or ``wide SV'' region), following the ecliptic plane from dense regions of the Galactic Bulge through low-dust regions of the planned LSST Wide-Fast-Deep footprint.
Approximately halfway through the SV period, the footprint was consolidated to $\sim$750~deg$^{2}$ to concentrate visits and improve coadd depth; in the final week of commissioning it was further reduced to $\sim$300~deg$^{2}$ to achieve the visit density required for difference-image testing.
As a result, the visit count and coadded depth increase substantially toward the inner regions of the footprint: the wide SV (3K) 3,000~deg$^{2}$ region reaches a median of 38 visits across all bands and an estimated coadded depth of $m_5\approx$ 25.0, the 750 deg$^{2}$ subset reaches 56 visits and $m_5\approx$ 25.8, and the 300 deg$^{2}$ core reaches 64 visits and $m_5$ \approx 25.7 (Table~\ref{tab:sv_wide_configs}).
For comparison, the LSST Year~1 WFD baseline predicts a median of 13 visits per pointing and $m_5 \approx$ 25.0 over the full footprint; the DP2 wide-area coadds therefore reach or exceed Year~1 LSST depth across the consolidated regions of the SV footprint.
...
Each of the LSST Deep Drilling Fields were observed during commissioning, although the depth and completeness  varied between fields.
...
\subsection{Start of Operations and the Start of the LSST}
\label{ssec:ops-lsst-start}

The \textit{Start of Rubin Operations} and the \textit{Start of the LSST} are two distinct milestones separated by the \textit{Rubin Operations Early Optimization Phase}.
Understanding the distinction between these two milestones, and what is required to transition from one to the other, is essential context for the Early Science Program.

Following Rubin Construction substantial completeness (\S\ref{ssec:commissioning-schedule}) on \cstrcompdate, Rubin Operations began the following day on \opsstartdate.
This milestone marked the formal handover of Rubin Observatory from the Construction project to the Operations team.
The \textit{Start of the LSST} is the subsequent milestone at which Rubin Observatory begins executing the Legacy Survey of Space and Time, as defined by the baseline survey strategy \citep{PSTN-056}.

The period from the Start of Rubin Operations through the Start of the LSST is termed the \textit{Early Operations Optimization Phase} (\S~\ref{ssec:defn}).
During Construction Commissioning, the as-built system demonstrated that it is capable of meeting the LSST science requirements as defined in the Science Requirements Document \citedsp{LPM-17}.
However, demonstrating that the system \textit{can} achieve the required performance is not the same as demonstrating that it can do so in a sustained, reliable, and repeatable manner over the course of a 10-year survey.
The goal of the Early Optimization Phase is to bridge this gap: to bring the system to a state of operational readiness that supports start of the LSST.
It is expected that optimization will continue even after the start of the LSST.

Data collected during the Early Optimization Phase continue to provide the basis for early data products released to the community.
For example, early alerts based on templates built during the Commissioning period began streaming to community brokers on \rubinfirstalerts\footnote{\url{https://rubinobservatory.org/news/first-alerts}}, and the release of nightly Processed Visit Images (PVIs) is expected sometime during \npcdate.

At the time of this writing, Rubin Observatory remains in the Early Optimization Phase.
A start date for the LSST will be determined once that phase is successfully completed.