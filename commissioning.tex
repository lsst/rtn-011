...
\begin{figure}[htb]
\centering
\includegraphics[width=0.98\linewidth]{rubin_early_science_schematic}
...
Following Rubin Construction completeness (\S\ref{ssec:commissioning-schedule}) on \cstrcompdate, Rubin Operations began the following day on \opsstartdate.
...
The period from the Start of Rubin Operations through the Start of the LSST is termed the \textit{Rubin Operations Early Optimization Phase} (\S~\ref{ssec:defn}).
...
The goal of the Early Optimization Phase is to bridge this gap: to bring the system to a state of operational readiness in which it can execute the LSST within the specifications of the SRD, night after night, without interruption.
Once stable and repeatable performance at the required level has been achieved, the LSST can begin.
...
For example, early alerts based on templates built during the Commissioning period began streaming to community brokers on \rubinfirstalerts\footnote{\url{https://rubinobservatory.org/news/first-alerts}}, and the release of nightly Processed Visit Images (PVIs) is expected from \nightlypvis.
 
At the time of writing, Rubin Observatory remains in the Early Optimization Phase and the LSST has not yet begun.
A start date for the LSST has not yet been determined.

%\subsection{Start of Operations and the LSST}
%\label{ssec:ops-lsst-start}
%
%Rubin Construction Complete: The point at which Rubin Observatory was deemed sufficiently complete and functional to be handed over from Construction to Operations for early
%system optimization and preparation for the start of the LSST survey. This milestone was
%achieved on 2025-10-24.
%
%The period from the start of Rubin Operations, 2025-10-25, through
%the first Data Release, DR1
%
%The period from the start of Rubin Operations, 2025-10-25, through the start of the LSST is termed the Rubin Operations Early Optimization Phase
%
%During this period  the Commissioning system that demonstrated that the as-built system can meet the LSST science requirements, and the  Operations team is now focused on achieving stable, repeatable nightly performance while
%coordinating remaining activities with the Construction team in preparation for full survey
%operations .. is focused on getting the system to a sustainable and reiable state to execute the LSST within the specifications in the Science Requirements Document (SRD). 
%
%The key parameters that are being optimised at the image quality and the survey speed, as represented in fugure x below. 
%
%Although commissoning demonstrated that the system was capable of delivring data in the range indicated, it does not do it in a sustained relent;ess reliable manner that is necessary to deliver a 10-year LSST dataset for science described in the SRD.  The goal of early operations optimization is to get the system to this point.  Once we are there, the LSST will start.  
%
%The data collected during this period will continue to provide the basis for early data products that can be released to the community. 
%
%For example -- early alerts based on templates built during the commissioning period. (refer to https://rubinobservatory.org/news/first-alerts)
