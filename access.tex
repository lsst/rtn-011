...
There will be an important change to the Butler for DP1. 
To date, science users have accessed Rubin data products on the RSP using a  full read-write \textit{Direct Butler}, which connects to an SQL database.
From DP1, for both scaling and security reasons, science users will access the  Rubin data products  via a new \textit{Remote Butler}, which interacts with a server via a REST API. 
The  \textit{Remote Butler} read and query API will be the same as for the DP0-era \textit{Direct Butler}, however the new \textit{Remote Butler} will not  support user write operations.  
Users will be able to write the outputs of their analyses to local space, and Rubin anticipates providing some tooling to support local write. 
Various options. 
Users will also be able to set up a local sqlite Butler to which they can write the outputs of their analysis.  
More details on setting up a local writeable Butler can be found in \citeds{dmtn-303}.
